import { ADAO_EVA_SCENES } from './src/data/adaoEvaScript.js';
...
async function generateSceneAudio(dialogue: string, characterName: string): Promise<Buffer | null> {
  ...
  const voiceConfig: Record<string, { voice: string; style: string; rate?: string; pitch?: string }> = {
    ...
    'serpent': { voice: 'pt-BR-AntonioNeural', style: 'whispering', rate: '-5%', pitch: '+10Hz' },
  };
...
async function buildFinalEpisodeEp2(
  ...
  return new Promise(async (resolve, reject) => {
    try {
      const listVoiceFile = 'concat_voice_list_ep2.txt';
      const listVoiceContent = tempVoiceFiles.map(f => `file '${f.replace(/'/g, "'\\''")}'`).join('\n');
      fs.writeFileSync(listVoiceFile, listVoiceContent);

      const tempVoicePath = 'temp_voice_track_ep2.mp3';
      const tempMixedPath = 'temp_mixed_main_ep2.mp3';
      ...
      console.log('🎬 3. Adicionando Abertura, Transições e Encerramento (Re-encodando)...');
...
async function generateEpisode2Locally() {
  console.log(' ============================================');
  console.log(' GERANDO ÁUDIO: EPISÓDIO 2 - ADÃO E EVA');
  console.log('🎬 ============================================\n');

  const scenes = ADAO_EVA_SCENES.filter(s => s && s.sceneNumber);
  const tempFiles: string[] = [];
  let idx = 0;

  console.log('🎙️ Gerando voz de transição...');
  const transitionText = "Olá meus amiguinhos! Vamos juntos descobrir a história de Adão e Eva!";
  const transitionVoiceBuffer = await generateSceneAudio(transitionText, 'narrator');
  if (transitionVoiceBuffer) {
    fs.writeFileSync('ep2_transition_voice.mp3', transitionVoiceBuffer);
  }

  for (const scene of scenes) {
    console.log(`\n🎬 Cena ${scene.sceneNumber}: ${scene.title}`);
    for (const line of scene.lines) {
        console.log(`🎙️ [${line.character}]: "${line.text.substring(0, 60)}..."`);
        
        const audioBuffer = await generateSceneAudio(line.text, line.character);

        if (audioBuffer) {
          const tempFile = `ep2_fix_${idx++}.mp3`;
          fs.writeFileSync(tempFile, audioBuffer);
          tempFiles.push(tempFile);
          console.log(`   ✅ Áudio salvo (${(audioBuffer.length / 1024).toFixed(1)} KB)`);
        }
  ...
  const finalOutputFile = 'episode02-final-bgm-full.mp3';
  
  // A NOVA ABERTURA PADRÃO DAS 4 TEMPORADAS
  ...
  const transitionVoicePath = 'ep2_transition_voice.mp3';
  ...
  await buildFinalEpisodeEp2(tempFiles, finalOutputFile, bgmPath, introPath, transitionVoicePath, transitionSfxPath, outroPath);

  console.log('☁️ Fazendo upload...');
  const finalBuffer = fs.readFileSync(finalOutputFile);
  const fullAudioPath = 'season1/episode02/episode02-completo.mp3';
  ...
  console.log('🎉 EPISÓDIO GERADO!');
  console.log(`🔗 URL: ${fullAudioUrl}`);
}

generateEpisode2Locally().catch(console.error);
