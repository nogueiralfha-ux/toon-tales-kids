...
export const AudioPlayerControls: React.FC<AudioPlayerControlsProps> = ({
  ...
  return (
    <div id="audio-player-controls" className="w-full bg-white border-2 border-slate-100 rounded-3xl p-5 sm:p-7 shadow-md select-none">
      ...
      <div className="mb-5">
        <div className="flex items-center justify-between text-xs font-bold text-slate-500 mb-2">
          ...
          <div className="font-mono text-slate-600 font-semibold">
            {formatTime(elapsedDurationSec)} / {formatTime(totalDurationSec)} <span className="text-slate-400 font-normal">(Alvo: 8–15 min)</span>
          </div>
        </div>
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2 sm:gap-3">
          ...
          <button
            ...
            className={`px-8 py-3.5 rounded-2xl flex items-center gap-2.5 font-black font-brand text-base sm:text-lg shadow-lg transition-all duration-200 active:scale-95 ${
              isPlaying
      ...
      {showTroubleshoot && (
        <div className="mt-3 p-4 rounded-2xl bg-amber-50 border border-amber-200 text-slate-700 text-xs space-y-2 animate-fade-in">
          ...
          <ol className="list-decimal list-inside space-y-1 text-slate-700 font-medium pl-1">
            <li><strong>Permissão de Reprodução do Navegador:</strong> Navegadores modernos bloqueiam áudio automático até que você clique em <em>"OUVIR"</em> ou <em>"Testar Som & Voz"</em> na página.</li>
