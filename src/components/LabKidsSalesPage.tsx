...
export const LabKidsSalesPage: React.FC<LabKidsSalesPageProps> = ({
  ...
  return (
    <div className="min-h-screen bg-[#FDFBF7] text-slate-800 font-sans selection:bg-purple-600 selection:text-white pb-16">
      ...
      {/* ------------------------------------------------------------- */}
      {/* 2. CABEÇALHO DA PÁGINA COM SELETOR DE PRODUTOS                */}
      {/* ------------------------------------------------------------- */}
      <header className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 flex items-center justify-between gap-4 border-b border-purple-100">
        ...
        <div className="flex items-center gap-3">
          {onOpenBiblicalSales && (
            <button
              onClick={onOpenBiblicalSales}
              className="px-3.5 py-2 rounded-xl text-xs font-bold text-slate-600 hover:text-orange-600 hover:bg-orange-50 border border-slate-200 transition-colors hidden sm:flex items-center gap-1.5"
            >
              <span>📖 Ver Histórias Bíblicas</span>
            </button>
          )}

          <button
            onClick={onEnterPlatform}
            className="px-4 py-2 rounded-xl bg-purple-100 hover:bg-purple-200 text-purple-900 text-xs font-black uppercase tracking-wider transition-all"
          >
            Acessar Plataforma
          </button>
          ...
          <a
            href="#oferta"
            className="px-5 py-2.5 rounded-xl bg-gradient-to-r from-emerald-500 to-teal-500 hover:from-emerald-400 hover:to-teal-400 text-white text-xs font-black uppercase tracking-wider shadow-md shadow-emerald-400/30 hover:scale-105 active:scale-95 transition-all flex items-center gap-1"
          >
